...
public class Block implements SeekingIterable<Slice, Slice> {
	...
	public Block(Slice block, Comparator<Slice> comparator) {
		...
		// Keys are prefix compressed. Every once in a while the prefix
		// compression is restarted and the full key is written.
		// These "restart" locations are written at the end of the file, so you
		// can seek to key without having to read the
		...
		if (restartCount > 0) {
			...
			checkArgument(restartOffset < block.length() - SIZE_OF_INT,
