...
public final class DbIterator extends AbstractSeekingIterator<InternalKey, Slice> implements InternalIterator {
	 ...
	 * iterator methods. Before committing changes to this code, make sure that
	 * the performance of the DB benchmark with the following parameters has not
	 * regressed:
	 *
	 ...
	 * interface, but instead used the concrete implementations. This is because
	 * we want the hot spot compiler to inline the code from the concrete
	 * iterators, and this can not happen with truly polymorphic call-sites. If
	 * a future version of hot spot supports inlining of truly polymorphic
	 * call-sites, this code can be made much simpler.
	 */
	...
	public DbIterator(MemTableIterator memTableIterator, MemTableIterator immutableMemTableIterator,
			List<InternalTableIterator> level0Files, List<LevelIterator> levels, Comparator<InternalKey> comparator) {
	...
	private void resetPriorityQueue() {
		...
		if (immutableMemTableIterator != null && immutableMemTableIterator.hasNext()) {
			heapAdd(new ComparableIterator(immutableMemTableIterator, comparator, i++,
	...
	private static class ComparableIterator
		...
		private ComparableIterator(SeekingIterator<InternalKey, Slice> iterator, Comparator<InternalKey> comparator,
