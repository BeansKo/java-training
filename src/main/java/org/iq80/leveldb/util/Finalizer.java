...
public class Finalizer<T> {
	...
	public synchronized void addCleanup(T item, Callable<?> cleanup) {
		...
		// create a reference to the item so we are notified when it is garbage
		// collected
		FinalizerPhantomReference<T> reference = new FinalizerPhantomReference<>(item, referenceQueue, cleanup);

		// we must keep a strong reference to the reference object so we are
		// notified when the item
		// is no longer reachable (if the reference object is garbage collected
		// we are never notified)
	...
	private class FinalizerQueueProcessor implements Runnable {
		...
		public void run() {
			while (!destroyed.get()) {
				...
				// remove the reference object itself from our list of
				// references
