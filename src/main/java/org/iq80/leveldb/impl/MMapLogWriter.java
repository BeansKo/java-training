...
public class MMapLogWriter implements LogWriter {
	...
	public synchronized void close() throws IOException {
		...
		Closeables.closeQuietly(accessFile);
	}
	...
	// Writes a stream of chunks such that no chunk is split across a block
	// boundary
	@Override
	public synchronized void addRecord(Slice record, boolean force) throws IOException {
		...
		// Fragment the record int chunks as necessary and write it. Note that
		// if record
		...
		do {
			...
			// Invariant: we never leave less than HEADER_SIZE bytes available
			// in a block
			int bytesAvailableInBlock = bytesRemainingInBlock - HEADER_SIZE;
			checkState(bytesAvailableInBlock >= 0);

			// if there are more bytes in the record then there are available in
			// the block,
